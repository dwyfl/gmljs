  ...
  initDefault() {
    for (var i in this.supportedChildNodes) {
      if (this.supportedChildNodes[i].hasOwnProperty('model')) {
        ...
        if (!this.supportedChildNodes[i].hasOwnProperty('array') || this.supportedChildNodes[i].array) {
          this[i] = [this[i]];
        }
      }
    ...
    Object.keys(this.supportedAttributes).forEach(key => {
      if (this.supportedAttributes[key].hasOwnProperty('default')) {
        this[key] = this.supportedAttributes[key].default;
      }
    });
  }
  getAttribute(key) {
